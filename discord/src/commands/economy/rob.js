...
module.exports = class RobCommand extends Command {
    ...
    async run (message) {
        ...
        if (user.robTime > Date.now()) {
            message.reply('commands.rob.has_been_robbed', { remaining: parseInt(user.robTime / 1000) })
        }
        else {
            if (!user.usingWeapon.weapon) return message.reply('commands.rob.need_weapon')
            ...
            if (result == message.author.id) {
                ...
                user.save()
                victim.save()
                message.reply('commands.rob.success', { value: value.toLocaleString(message.guild.db.lang === 'en' ? 'en-US' : 'pt-BR') })
            }
            else {
                ...
                user.save()
                bank.save()
                message.reply('commands.rob.fail', { value: (5000).toLocaleString(message.guild.db.lang === 'en' ? 'en-US' : 'pt-BR') })
            }
        }
    }
}